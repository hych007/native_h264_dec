                                  ...
                                  int* bytesUsed)
{
    ...
    // If parsing fail (probably no PPS/SPS), continue anyway it may arrived
    // later (happen on truncated streams).
    int fieldType;
    int sliceType;
    ...
    , m_execParams()
{
    ...
    // m_execParams
    memset(m_managedBufDesc.get(), 0,
           sizeof(m_managedBufDesc[0]) * decodeBufDescCount);
    memset(&m_execParams, 0, sizeof(m_execParams));
    m_execParams.pCompressedBuffers = m_managedBufDesc.get();

    // m_sliceLong/m_sliceShort
    DXVA_Slice_H264_Long emptySliceLong = {0};
    m_sliceLong.resize(maxSlices, emptySliceLong);
...
bool CH264DXVA2Decoder::Init(const DDPIXELFORMAT& pixelFormat,
                             int64 averageTimePerFrame)
{
    assert(m_accel);
...
HRESULT CH264DXVA2Decoder::Decode(const void* data, int size, int64 start,
                                  int64 stop, IMediaSample* outSample,
                                  int* bytesUsed)
{
    assert(data);
...
//     h264_detail::SetCurrentPicIndex(surfaceIndex, &m_picParams, getPreDecode());
    ...
    HRESULT r = beginFrame(outSample);
    if (FAILED(r))
        return r;
    ...
    // Send picture parameters
    r = allocExecBuffer(DXVA2_PictureParametersBufferType, 0, &m_picParams,
                        sizeof(m_picParams), NULL);
    if (FAILED(r))
        return r;
    ...
    // Add bitstream, slice control and quantization matrix.
    void* DXVABuffer = NULL;
    r = allocExecBuffer(DXVA2_BitStreamDateBufferType, 0, NULL, 0, &DXVABuffer);
    if (FAILED(r))
        return r;
    ...
    int execBufSize =
        m_useLongSlice ?
            sizeof(m_sliceLong[0]) * slice :
            sizeof(m_sliceShort[0]) * slice;
    r = allocExecBuffer(DXVA2_SliceControlBufferType, 0, execBuf,
                        execBufSize, NULL);
    if (FAILED(r))
        return r;

    r = allocExecBuffer(DXVA2_InverseQuantizationMatrixBufferType, 0,
                        &scalingMatrix, sizeof(scalingMatrix), NULL);
    if (FAILED(r))
        return r;
...
HRESULT CH264DXVA2Decoder::getFreeSurfaceIndex(
    int* surfaceIndex, intrusive_ptr<IMediaSample>* sampleToDeliver)
{
    assert(surfaceIndex);
    assert(sampleToDeliver);
...
HRESULT CH264DXVA2Decoder::beginFrame(IMediaSample* sample)
{
    intrusive_ptr<IMFGetService> getService;
    HRESULT r = sample->QueryInterface(IID_IMFGetService,
                                       reinterpret_cast<void**>(&getService));
    if (FAILED(r))
        return r;

    intrusive_ptr<IDirect3DSurface9> surface;
    r = getService->GetService(MR_BUFFER_SERVICE, IID_IDirect3DSurface9,
    ...
    for (int i = 0; i < 20; ++i)
    {
        TRY_EXECUTE(m_accel->BeginFrame(surface.get(), NULL));
        if (SUCCEEDED(r))
            break;
...
HRESULT CH264DXVA2Decoder::endFrame()
{
    return m_accel->EndFrame(NULL);
}

HRESULT CH264DXVA2Decoder::allocExecBuffer(int compType, int bufIndex,
                                           const void* nonBitStreamData,
                                           int size, void** DXVABuffer)
{
    void* allocated;
    UINT allocatedSize;
    HRESULT r = m_accel->GetBuffer(compType, &allocated, &allocatedSize);
    if (FAILED(r))
        return r;

    assert(static_cast<int>(allocatedSize) >= size);
    if (static_cast<int>(allocatedSize) < size)
        return E_FAIL;
    ...
    DXVA2_DecodeBufferDesc& desc =
        m_execParams.pCompressedBuffers[m_execParams.NumCompBuffers];
    desc.CompressedBufferType = compType;
    desc.DataSize = size;
    desc.NumMBsInBuffer = 0;

    m_execParams.NumCompBuffers++;
    return r;
}

inline void CH264DXVA2Decoder::reviseLastDataSize(int size)
{
    int n = m_execParams.NumCompBuffers;
    if (n)
        m_execParams.pCompressedBuffers[n - 1].DataSize = size;
}

HRESULT CH264DXVA2Decoder::execute()
{
    for (int i = 0; i < static_cast<int>(m_execParams.NumCompBuffers); ++i)
    {
        HRESULT r2 = m_accel->ReleaseBuffer(
            m_execParams.pCompressedBuffers[i].CompressedBufferType);
        assert(SUCCEEDED(r2));
    }

    HRESULT r = m_accel->Execute(&m_execParams);
    m_execParams.NumCompBuffers = 0;
    return r;
}

bool CH264DXVA2Decoder::updateRefFrameSliceLong(int slice, int dataOffset,
                                                int sliceLength)
{
    if (slice >= static_cast<int>(m_sliceLong.size()))
        return false;
...
bool CH264DXVA2Decoder::updateRefFrameSliceShort(int slice, int dataOffset,
                                                 int sliceLength)
{
    if (slice >= static_cast<int>(m_sliceShort.size()))
        return false;
...
int CH264DXVA2Decoder::buildBitStreamAndRefFrameSlice(const void* data,
                                                      int size, void* dest)
{
    assert(data);
    assert(dest);
    ...
    bool (CH264DXVA2Decoder::*updateFunc)(int, int, int) =
        m_useLongSlice ?
            &CH264DXVA2Decoder::updateRefFrameSliceLong :
            &CH264DXVA2Decoder::updateRefFrameSliceShort;
    ...
    m_sliceLong[slice - 1].SliceBytesInBuffer += padding;
    m_sliceShort[slice - 1].SliceBytesInBuffer += padding;
    reviseLastDataSize(dataOffset + padding);
    return slice;
}